...
import dev.riley.theboysmod.client.animation.AustinEntityAnimation;
import dev.riley.theboysmod.client.animation.BryanEntityAnimation;
...
public class BryanEntityModel extends HierarchicalModel<BryanEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "bryan_entity"), "main");


    private final BryanEntityModel.ModelParts parts;


    public BryanEntityModel(ModelPart root) {
        ...
        this.parts = new BryanEntityModel.ModelParts(body, head);
    }
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        body.addOrReplaceChild("leftArm", CubeListBuilder.create().texOffs(32, 48).addBox(-1.0F, -2.0F, -2.0F, 4.0F, 12.0F, 4.0F, new CubeDeformation(0.0F))
        ...
        body.addOrReplaceChild("leftLeg", CubeListBuilder.create().texOffs(16, 48).addBox(-2.0F, 0.0F, -2.0F, 4.0F, 12.0F, 4.0F, new CubeDeformation(0.0F))
    ...
    @Override
    public void setupAnim(@NotNull BryanEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, BryanEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(BryanEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
