...
import dev.riley.theboysmod.client.animation.JohnEntityAnimation;
...
public class JohnEntityModel extends HierarchicalModel<JohnEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "john_entity"), "main");


    private final JohnEntityModel.ModelParts parts;


    public JohnEntityModel(ModelPart root) {
        ...
        this.parts = new JohnEntityModel.ModelParts(body, head);
    }
    ...
    @Override
    public void setupAnim(@NotNull JohnEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, JohnEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(JohnEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
