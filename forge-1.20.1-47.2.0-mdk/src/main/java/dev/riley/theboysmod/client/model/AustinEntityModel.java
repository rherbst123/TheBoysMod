...
public class AustinEntityModel extends HierarchicalModel<AustinEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "austin_entity"), "main");


    private final AustinEntityModel.ModelParts parts;


    public AustinEntityModel(ModelPart root) {
        ...
        this.parts = new AustinEntityModel.ModelParts(body, head);
    }
    ...
    @Override
    public void setupAnim(@NotNull AustinEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, AustinEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(AustinEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
