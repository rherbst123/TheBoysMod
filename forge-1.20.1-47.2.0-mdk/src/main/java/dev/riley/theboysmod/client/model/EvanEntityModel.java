...
import dev.riley.theboysmod.client.animation.EvanEntityAnimation;
...
public class EvanEntityModel extends HierarchicalModel<EvanEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "evan_entity"), "main");


    private final EvanEntityModel.ModelParts parts;


    public EvanEntityModel(ModelPart root) {
        ...
        this.parts = new EvanEntityModel.ModelParts(body, head);
    }
    ...
    @Override
    public void setupAnim(@NotNull EvanEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, EvanEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(EvanEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
