...
import dev.riley.theboysmod.client.animation.NickEntityAnimation;
...
public class NickEntityModel extends HierarchicalModel<NickEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "nick_entity"), "main");


    private final NickEntityModel.ModelParts parts;


    public NickEntityModel(ModelPart root) {
        ...
        this.parts = new NickEntityModel.ModelParts(body, head);
    }
    ...
    @Override
    public void setupAnim(@NotNull NickEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, NickEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(NickEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
