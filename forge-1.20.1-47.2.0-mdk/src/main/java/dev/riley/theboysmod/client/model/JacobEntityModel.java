...
import dev.riley.theboysmod.client.animation.JacobEntityAnimation;
...
public class JacobEntityModel extends HierarchicalModel<JacobEntity> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TheBoysMod.MODID, "jacob_entity"), "main");


    private final JacobEntityModel.ModelParts parts;


    public JacobEntityModel(ModelPart root) {
        ...
        this.parts = new JacobEntityModel.ModelParts(body, head);
    }
    ...
    @Override
    public void setupAnim(@NotNull JacobEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        root().getAllParts().forEach(ModelPart::resetPose);
        animate(entity.idleAnimationState, JacobEntityAnimation.IDLE, ageInTicks);
        // animate(entity.attackAnimationState, ExampleAnimatedEntityAnimation.ATTACK, ageInTicks);
        // etc...

        if(!entity.isInWaterOrBubble()) {
            animateWalk(JacobEntityAnimation.WALKING, limbSwing, limbSwingAmount, 1.0F, 2.5F);
        } else {
            // TODO: Swim animation
        }
    }
